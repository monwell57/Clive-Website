// Path: app/resources/ResourcesClient.tsx
...
// Slugify title for the download filename.
...
// Sanity's CDN honors `?dl=<filename>` by returning a
// Content-Disposition: attachment header, which is what actually makes
// the browser download the file (the HTML5 `download` attribute is
// ignored cross-origin without it).
function getResourceUrl(resource: Resource): string | null {
	switch (resource.resourceType) {
		case 'pdf': {
			const url = resource.file?.asset?.url;
			if (!url) return null;
			const filename = `${toDownloadFilename(resource.title)}.pdf`;
									...
									<div className="p-7 flex flex-col flex-1">
										...
										{url && (
											<a
												...
												download={
													resource.resourceType === 'pdf'
														? `${toDownloadFilename(resource.title)}.pdf`
														: undefined
												}
